...
### Playing nice with all containers

The goal of this project is to offer a viable solution for **agnostic container configuration**. There is a wide variety of containers, and they can have very different behaviour.
In regard to service providers, it is important to understand there are 2 classes of containers:

- *runtime containers*
- *compiled/cached containers*

Runtime containers will typically instantiate service providers on each request. The `getServices` method will be called on each request to configure the container.
...
In certain special conditions, compiled/cached containers can optimize call to factories.
...
When asked the `my_service` service, a cached/compiled container will have to instantiate the service provider, then call the `getServices` method.

In PHP, the minimal way to fetch the service is:

```php
$serviceProvider = new MyServiceProvider();
$factories = $serviceProvider->getServices();
$service = $factories['my_service']();
```

Now, let's have a look at this similar code:
...
Compiled/cached container can now run `getServices()` once and subsequently directly refer to the factory method (`MyServiceProvider::createMyService`).

Now, the minimal code to fetch the service becomes:

```php
$service = MyServiceProvider::createMyService()();
```

So the second solution offers more room for optimization for compiled/cached containers.

As a courtesy for users looking for the best performance, service providers may consider using `public static function`s as factories.
